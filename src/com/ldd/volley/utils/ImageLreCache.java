...
class ImageLreCache extends LruCache<String, Bitmap> implements ImageLoader.ImageCache {
    ...
    public ImageLreCache(int maxSize,String diskCacheFodler,int diskCacheSize) {
        ...
        try {
        	mDiskLruCache = DiskLruCache.open(getDiskCacheDir(VolleyApplication.getInstance(),CACHE_FOLDER_NAME),
					getAppVersion(VolleyApplication.getInstance()) , 1, DISK_CACHE_SIZE);
		} catch (IOException e) {
			e.printStackTrace();
		}
    }
